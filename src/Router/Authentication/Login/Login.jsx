...
import { LoginWithGoogle, LoginWithFacebook, LoginWithEmailAndPassword } from '../../../Utility/firebase'
...
const Login = () => {
  ...
  return (
    <div  className='bg-cover bg-center min-xl:h-fit py-6' style={{ backgroundImage: `url(${BG})`}}>
      <h1 className='text-white text-[40px] max-md:mx-[70px] font-bold mx-[180px] my-5'>Login Here</h1>
      ...
      <form onSubmit ={HandleSubmittion} className='flex flex-col gap-8 m-[22px] w-[30%] mx-[80px] max-md:w-[50%]'>
        <Input types="email" values={Email} placeholder='Enter your Username' changed={handleInputEmail}></Input>
        <Input types="password" values={Pin} placeholder='Enter your Password' changed={handleInputPin}></Input>
      <Button >Sign In</Button>
      </form>
      <p className='mx-[120px] text-lg'>Not has an Account Yet? <Link to='/Registration' className='text-white'>Create Account</Link> </p>
    </div>
  )
}
